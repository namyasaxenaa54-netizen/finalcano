...
const AdminDashboard = () => {
  ...
  const loadStats = async () => {
    try {
      const [{ count: totalBrands }, { count: totalProducts }] = await Promise.all([
        supabase.from('brands').select('*', { count: 'exact', head: true }),
        supabase.from('products').select('*', { count: 'exact', head: true })
      ]);

      const [canobond, blackberry, furniture] = await Promise.all([
        supabase.from('products').select('*, brands!inner(slug)', { count: 'exact', head: true }).eq('brands.slug', 'canobond'),
        supabase.from('products').select('*, brands!inner(slug)', { count: 'exact', head: true }).eq('brands.slug', 'blackberry'),
        supabase.from('products').select('*, brands!inner(slug)', { count: 'exact', head: true }).eq('brands.slug', 'furniture')
      ]);
      ...
      setStats({
        ...
        products_by_brand: {
          canobond: canobond.count,
          blackberry: blackberry.count,
